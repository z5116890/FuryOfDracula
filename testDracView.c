// testDracView.c ... test the DracView ADT

#include <stdio.h>
#include <stdlib.h>
#include <assert.h>
#include <string.h>
#include "DracView.h"

int main()
{
    int i;
    DracView dv;
    LocationID trail[TRAIL_SIZE] = {0};


    printf("Test for basic functions, just before Dracula's first move\n");
    PlayerMessage messages1[] = {"Hello","Rubbish","Stuff",""};
    dv = newDracView("GST.... SAO.... HZU.... MBB....", messages1);
    
    //round should be 0
    assert(giveMeTheRound(dv) == 0);
    
    //Should be unknown location for drac on turn 1
    printf("player: %d\n", whereIs(dv,PLAYER_LORD_GODALMING));
    assert(whereIs(dv,PLAYER_LORD_GODALMING) == STRASBOURG);
    assert(whereIs(dv,PLAYER_DR_SEWARD) == ATLANTIC_OCEAN);
    assert(whereIs(dv,PLAYER_VAN_HELSING) == ZURICH);
    assert(whereIs(dv,PLAYER_MINA_HARKER) == BAY_OF_BISCAY);
    assert(whereIs(dv,PLAYER_DRACULA) == UNKNOWN_LOCATION);
    assert(howHealthyIs(dv,PLAYER_DRACULA) == GAME_START_BLOOD_POINTS);
    
    //checking drac's trail on turn 1
    printf("trail for drac should be all UNKNOWN LOCATION on turn 1\n");
    giveMeTheTrail(dv, PLAYER_DRACULA, trail);
    for(int i = 0; i < TRAIL_SIZE; i++){
        printf("trail[%d] = %d\n", i, trail[i]);
        assert(trail[i] == UNKNOWN_LOCATION);
    }
    //checking score at beginning
    printf("score at the beginning is \n");
    assert(giveMeTheScore(dv) == GAME_START_SCORE);
    
    printf("no traps or vamps at any location on first turn\n");
    int numVamps = 0, numTraps = 0;
    for(int currentLoc = 0; currentLoc < MAX_MAP_LOCATION; currentLoc++){
        whatsThere(dv,1, &numTraps, &numVamps);
        assert(numTraps == 1);
        assert(numVamps == 0);
    }
    printf("yo\n");
    disposeDracView(dv);
    
    
    PlayerMessage rubbish[] = {"Hello","Rubbish","Stuff","","Mwahahah","Aha!","","",""};
    dv = newDracView("GED.... SGE.... HZU.... MCA.... DCF.V.. "
                     "GMN.... SCFVD.. HGE.... MLS.... DTOT... "
                     "GLO.... SMR.... HCF.... MMA.... DTOT... "
                     "GPL.... SMS.... HMR.... MGR.... DBAT... "
                     "GLO.... SBATD.. HMS.... MMA.... DHIT... "
                     "GPL.... SSJ.... HBA.... MGR.... DALT... "
                     "GPL.... SSJ.... HBA.... MGR...." , rubbish);
    
    //giveMeTheTrail should replace HIDE AND Dn with real location
    giveMeTheTrail(dv, PLAYER_DRACULA, trail);
    for(int i = 0; i < TRAIL_SIZE; i++){
        printf("Dractrail[%d] = %d\n", i, trail[i]);
    }
    
    
    
    
    
    
    
    
    
    
    
    //testing round 6 for function whereIs
    printf("MINA IS IN = %d\n", whereIs(dv,PLAYER_MINA_HARKER));
    printf("VAN HELSING IS IN = %d\n", whereIs(dv,PLAYER_VAN_HELSING));
    printf("SEWARD IS IN = %d\n", whereIs(dv,PLAYER_DR_SEWARD));
    printf("GOLDALMING IS IN = %d\n", whereIs(dv,PLAYER_LORD_GODALMING));
    printf("DRAC IS IN = %d\n", whereIs(dv,PLAYER_DRACULA));
    assert(whereIs(dv,PLAYER_LORD_GODALMING) == PLYMOUTH);
    assert(whereIs(dv,PLAYER_DR_SEWARD) == SARAJEVO);
    assert(whereIs(dv,PLAYER_VAN_HELSING) == BARCELONA);
    assert(whereIs(dv,PLAYER_MINA_HARKER) == GRANADA);
    assert(whereIs(dv,PLAYER_DRACULA) == ALICANTE);
    
    
    int dracSize = 0;
    //dractrail should be [ALICANTE(1), TOULOUSE(63), BARCELONA(5), TOULOUSE(63), BORDEAUX(11), CLERMONT FERRAND(18)]
    //testing round 6 for function whereCanIgo
    printf("testing whereCanIgo with one double back already in trail\n");
    //LocationID *dracPlaces = whereCanIgo(dv, &dracSize, 1,1);
    giveMeTheTrail(dv, PLAYER_DRACULA, trail);
    printf("dracsize = %d", dracSize);
    
    
    

    disposeDracView(dv);
    
    printf("testing whereCanIgo with one hide already in trail");
    dv = newDracView("GED.... SGE.... HZU.... MCA.... DCF.V.. "
                     "GMN.... SCFVD.. HGE.... MLS.... DBOT... "
                     "GLO.... SMR.... HCF.... MMA.... DTOT... "
                     "GPL.... SMS.... HMR.... MGR.... DBAT... "
                     "GLO.... SBATD.. HMS.... MMA.... DHIT... "
                     "GPL.... SSJ.... HBA.... MGR.... DALT... "
                     "GPL.... SSJ.... HBA.... MGR...." , rubbish);
    //dracTrail[AL,HIDE,BA,TO,BO,CF]
    dracSize = 0;
    LocationID *dracPlaces2 = whereCanIgo(dv, &dracSize, 1,1);
    assert(dracPlaces2[6] != ALICANTE);
    disposeDracView(dv);
    
    printf("testing whereCanIgo with one hide and one double back already in trail\n");
    dv = newDracView("GED.... SGE.... HZU.... MCA.... DCF.V.. "
                     "GMN.... SCFVD.. HGE.... MLS.... DBOT... "
                     "GLO.... SMR.... HCF.... MMA.... DTOT... "
                     "GPL.... SMS.... HMR.... MGR.... DBAT... "
                     "GLO.... SBATD.. HMS.... MMA.... DHIT... "
                     "GPL.... SSJ.... HBA.... MGR.... DD2T... "
                     "GPL.... SSJ.... HBA.... MGR.... DMAT..." , rubbish);
    
    dracSize = 0;
    LocationID *dracPlaces3 = whereCanIgo(dv, &dracSize, 1,1);
    
    LocationID trail2[TRAIL_SIZE];
    giveMeTheTrail(dv, PLAYER_DRACULA, trail2);

    for(int i = 0; i < TRAIL_SIZE; i++){
        for(int j = 0; j < dracSize; j++){
            assert(trail2[i] != dracPlaces3[j]);
        }
    }

    printf("passed\n");
    assert(dracPlaces3[6] != MADRID);
    disposeDracView(dv);
    
    //what traps and vamps are placed in location
    //int numT = 0, numV = 0;
    //whatsThere(dv,BARCELONA,&numT,&numV);
    //printf("numT = %d\n",numT);
    //assert(numT == 2 && numV == 0);
    //whatsThere(dv,CLERMONT_FERRAND,&numT,&numV);
    //assert(numT == 0 && numV == 1);
    
    
    
    printf("passed\n");


    
    printf("Test for encountering Dracula and hunter history\n");
    PlayerMessage messages2[] = {"Hello","Rubbish","Stuff","","Mwahahah","Aha!","","",""};
    dv = newDracView("GST.... SAO.... HCD.... MAO.... DGE.... "
                     "GGED... SAO.... HCD.... MAO....", messages2);
    printf("round = %d", giveMeTheRound(dv) );
    assert(giveMeTheRound(dv) == 1);
    assert(whereIs(dv,PLAYER_DRACULA) == GENEVA);
    assert(howHealthyIs(dv,PLAYER_LORD_GODALMING) == 5);
    assert(howHealthyIs(dv,PLAYER_DRACULA) == 30);
    assert(whereIs(dv,PLAYER_LORD_GODALMING) == GENEVA);
    LocationID history[TRAIL_SIZE];
    giveMeTheTrail(dv,PLAYER_DRACULA,history);
    assert(history[0] == GENEVA);
    assert(history[2] == UNKNOWN_LOCATION);
    giveMeTheTrail(dv,PLAYER_LORD_GODALMING,history);
    assert(history[0] == GENEVA);
    assert(history[1] == STRASBOURG);
    assert(history[2] == UNKNOWN_LOCATION);
    giveMeTheTrail(dv,PLAYER_DR_SEWARD,history);
    assert(history[0] == ATLANTIC_OCEAN);
    assert(history[1] == ATLANTIC_OCEAN);
    assert(history[2] == UNKNOWN_LOCATION);
    printf("passed\n");        
    disposeDracView(dv);

    
    
    
    
    
    
    
    
    
    
    
    
    printf("Test for Dracula leaving minions\n");
    PlayerMessage messages3[] = {"Hello","Rubbish","Stuff","","Mwahahah","Aha!","","","","Drop a V","Party in Strasbourg","Party","Party","Party"};
    dv = newDracView("GGE.... SGE.... HGE.... MGE.... DED.V.. "
                     "GST.... SST.... HST.... MST.... DMNT... "
                     "GST.... SST.... HST.... MST....", messages3);
    //int nT, nV;

    
    //whatsThere(dv,EDINBURGH,&nT,&nV);
    //assert(nT == 0 && nV == 1);
    printf("yo\n");
    
    
    
    
    
    
    
    
    
    
    
    
    
    
    
    
    
    
    
    
    
    
    
    
   // whatsThere(dv,MANCHESTER,&nT,&nV);
    
    
  //  assert(nT == 1 && nV == 0);
    assert(whereIs(dv,PLAYER_DRACULA) == MANCHESTER);
    giveMeTheTrail(dv,PLAYER_DRACULA,history);
    assert(history[0] == MANCHESTER);
    assert(history[1] == EDINBURGH);
    assert(history[2] == UNKNOWN_LOCATION);
    giveMeTheTrail(dv,PLAYER_MINA_HARKER,history);
    assert(history[0] == STRASBOURG);
    assert(history[1] == STRASBOURG);
    assert(history[2] == GENEVA);
    assert(history[3] == UNKNOWN_LOCATION);
    printf("passed\n");
    disposeDracView(dv);

    printf("Test for connections\n");
    int size, seen[NUM_MAP_LOCATIONS], *edges;

    printf("Checking Galatz road connections\n");
    PlayerMessage messages5[] = {"Gone to Galatz"};
    dv = newDracView("GGA....", messages5);
    edges = whereCanTheyGo(dv,&size,PLAYER_LORD_GODALMING,1,0,0);
    memset(seen, 0, NUM_MAP_LOCATIONS*sizeof(int));
    for (i = 0; i< size ; i++) seen[edges[i]] = 1;
    assert(size == 5); assert(seen[GALATZ]); assert(seen[CONSTANTA]);
    assert(seen[BUCHAREST]); assert(seen[KLAUSENBURG]); assert(seen[CASTLE_DRACULA]);
    free(edges);
    disposeDracView(dv);

    printf("Checking Ionian Sea sea connections\n");
    PlayerMessage messages6[] = {"Sailing the Ionian"};
    dv = newDracView("GIO....", messages6);
    edges = whereCanTheyGo(dv,&size,PLAYER_LORD_GODALMING,0,0,1);
    memset(seen, 0, NUM_MAP_LOCATIONS*sizeof(int));
    for (i = 0; i < size; i++) seen[edges[i]] = 1;
    assert(size == 7); assert(seen[IONIAN_SEA]); assert(seen[BLACK_SEA]);
    assert(seen[ADRIATIC_SEA]); assert(seen[TYRRHENIAN_SEA]);
    assert(seen[ATHENS]); assert(seen[VALONA]); assert(seen[SALONICA]);
    free(edges);
    disposeDracView(dv);

    printf("Checking Athens rail connections (none)\n");
    PlayerMessage messages7[] = {"Leaving Athens by train"};
    dv = newDracView("GAT....", messages7);
    edges = whereCanTheyGo(dv,&size,PLAYER_LORD_GODALMING,0,1,0);
    assert(size == 1);
    assert(edges[0] == ATHENS);
    free(edges);
    disposeDracView(dv);

    printf("passed\n");
    return 0;
}

